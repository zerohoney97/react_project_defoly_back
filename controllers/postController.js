...
exports.detailBoard = async (req, res) => {
  ...
  const commentdata = await Comment.findAll({
    where: { board_id: id },
    include: [{ model: Recomment }, { model: User }, { model: LikeComment }],
  });
  const newComment = commentdata.map((value) => {
    ...
    return {
      ...value.dataValues,
      User: value.User.dataValues.nickname,
      Img: value.User.dataValues.profile_img,
      LikeComments: temp,
    };
  });
  const realNewComment = newComment.map(async (value) => {
    const recommentdata = await Recomment.findAll({
    ...
    const newReComment = recommentdata.map((revalue) => {
      ...
      return {
        ...revalue.dataValues,
        User: revalue.User.dataValues.nickname,
        Img: revalue.User.dataValues.profile_img,
      };
    });
  ...
  res.json({
    ...
    commentdata: realRealNewComment,
    realNewComment,
  });
